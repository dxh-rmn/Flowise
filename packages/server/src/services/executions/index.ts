...
const getExecutionById = async (executionId: string, userId?: string): Promise<Execution | null> => {
    try {
...
const getAllExecutions = async (filters: ExecutionFilters = {}): Promise<{ data: Execution[]; total: number }> => {
    try {
        const appServer = getRunningExpressApp()
        const { id, agentflowId, agentflowName, sessionId, state, startDate, endDate, page = 1, limit = 12, userId } = filters
        ...
        if (userId) queryBuilder.andWhere('execution.userId = :userId', { userId })
...
const updateExecution = async (executionId: string, data: Partial<Execution>, userId?: string): Promise<Execution | null> => {
    try {
        const appServer = getRunningExpressApp()
 ...
 * @param userId Optional workspace ID to filter executions
 * @returns Object with success status and count of deleted executions
 */
const deleteExecutions = async (executionIds: string[], userId?: string): Promise<{ success: boolean; deletedCount: number }> => {
    try {
        const appServer = getRunningExpressApp()
        const executionRepository = appServer.AppDataSource.getRepository(Execution)
