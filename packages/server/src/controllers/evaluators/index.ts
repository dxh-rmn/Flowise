...
const getAllEvaluators = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!userId) {
            throw new InternalFlowiseError(StatusCodes.NOT_FOUND, `Error: evaluatorService.getAllEvaluators - user ${userId} not found!`)
        }
...
const getEvaluator = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userId = req.user?.id
        if (!userId) {
            throw new InternalFlowiseError(StatusCodes.NOT_FOUND, `Error: evaluatorService.getEvaluator - user ${userId} not found!`)
        }
...
const updateEvaluator = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userId = req.user?.id
        if (!userId) {
            throw new InternalFlowiseError(StatusCodes.NOT_FOUND, `Error: evaluatorService.updateEvaluator - user ${userId} not found!`)
        }
...
const deleteEvaluator = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userId = req.user?.id
        if (!userId) {
            throw new InternalFlowiseError(StatusCodes.NOT_FOUND, `Error: evaluatorService.deleteEvaluator - user ${userId} not found!`)
        }
